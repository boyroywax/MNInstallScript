...
function doBalance(message, tipper) {
  boxy.getBalance(tipper, 1, function(err, balance) {
    if (err) {
      message.reply('Error getting FoundChain (FCHAIN) Coin balance.').then(message => message.delete(5000));
    } else {
      message.reply('You have **' + balance + ' FoundChain (FCHAIN) Coin** :moneybag:');
    }
  });
}
...
function doDeposit(message, tipper) {
  getAddress(tipper, function(err, address) {
    if (err) {
      message.reply('Error getting your FoundChain (FCHAIN) Coin deposit address.').then(message => message.delete(5000));
    } else {
      message.reply('Your FoundChain (FOUND) address is ' + address);
    }
  });
}
...
function doWithdraw(message, tipper, words, helpmsg, cmdOffset) {
  ...
  if (amount === null) {
    message.reply("I don't know how to withdraw that many FoundChain (FCHAIN) Coins...").then(message => message.delete(5000));
    return;
  }
    ...
    if (amount < balance) {
      boxy.sendFrom(tipper, address, Number(amount-0.00001), function(err, txId) {
        if (err) {
          ...
          message.reply('You withdrew ' + amount + ' FoundChain (FCHAIN) to ' + address + '\n' + txLink(txId) + '\n');
        }
      });
    } else {
...
function doTip(bot, message, tipper, words, helpmsg, cmdOffset) {
  ...
  if (amount === null) {
    message.reply("I don't know how to tip that many FoundChain (FCHAIN) Coins...").then(message => message.delete(5000));
    return;
  }
  ...
  if (message.mentions.users.first().id) {
    boxy.getBalance(tipper, function(err, balance) {
      if (amount < balance) {
        sendTOUCH(bot, message, tipper, message.mentions.users.first().id.replace('!', ''), amount, prv);
      } else {
        message.reply("Account has insufficient funds").then(message => message.delete(5000));
      }
    });
...
function doSoakRainDrizzle(bot, message, tipper, words, helpmsg, tipType, cmdOffset) {
  ...
  if (amount === null) {
    ...
    message.reply("Atleast 0.1 FoundChain (FCHAIN) is required to rain").then(message => message.delete(5000));
    return;
  }
...
function soak(amount, online, callback){
  ...
  setUsernames.push(onlineUsername);
  callback(onlineID, onlineID.length + " users currently online. No FoundChain (FCHAIN) Coin is rained",
      ":thunder_cloud_rain: FoundChain (FCHAIN) Coins are falling from the sky!!! :thunder_cloud_rain: \n**" +
      amount/onlineID.length + " FoundChain (FCHAIN)** soaked on " + setUsernames[0], setUsernames);
}
...
function rain(amount, online, message, callback){
  ...
  callback(onlineID, "No new messages, since I woke up", ":cloud_rain: FoundChain (FCHAIN) Coins are raining from the sky!!! :cloud_rain: \n" +
      "**" + amount/onlineID.length + " FoundChain (FCHAIN)** rained down on " + onlineUsername + " :rocket:");
}
...
function drizzle(amount, online, message, callback){
  ...
  callback(onlineID,"No new messages, since I woke up", ":white_sun_rain_cloud: FoundChain (FCHAIN) Coins are drizzling from the sky!!! :white_sun_rain_cloud: \n" +
      "**" + amount/onlineID.length + " FoundChain (FCHAIN)** rained down on " + onlineUsername + " :rocket:");
}


function getPrice(bot, msg){
  getDataFromAPI("https://api.coingecko.com/api/v3/coins/foundchain", true, function(data){
    let result = JSON.parse(data);
    if (result !== "undefined") {
      if (result.market_data) {
        sendEmbedMessages(msg, "", 3447003, [{
          name: "FoundChain (FCHAIN) Market Price",
          value: "**" + result.market_data.current_price.btc + "** BTC/FOUND"
        }])
      } else {
        msg.reply("Price is not listed.")
      }
    }
  });
}
function getBlock(bot, msg){
  getDataFromAPI("http://104.238.184.36:3002/api/getblockcount", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0x00AE86, [{
          name: "FoundChain (FCHAIN) Coin - Current BLOCK",
...
function getWeight(bot, msg){
  getDataFromAPI("http://104.238.184.36:3002/api/getdifficulty", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0xF55555, [{
          name: "FoundChain (FCHAIN) Coin - Current Staking weight",
...
function getSupply(bot, msg){
  getDataFromAPI("http://104.238.184.36:3002/ext/getmoneysupply", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0xF55555, [{
          name: "FoundChain (FCHAIN) Coin - Supply",
          value: "Supply: **" + result + "** FCHAIN Coins"
        }])
      } else {
        msg.reply("Price is not set by buyers/sellers")
      }
...
function sendTOUCH(bot, message, tipper, recipient, amount, privacyFlag) {
  getAddress(recipient.toString(), function(err, address) {
    if (err) {
      message.reply("GET address Error: " +err.message).then(message => message.delete(5000));
    } else {
      boxy.sendFrom(tipper, address, Number(amount), 0, null, null, function(err, txId) {
        if (err) {
          ...
          if (privacyFlag) {
            ...
            var iimessage =
              ...
              txLink(txId) +
              '\n' +
              'DM me `$help` for FoundChain (FCHAIN) Coin Tipper instructions.';
            ...
            if (
              message.content.startsWith('!tiptigc private ')
            ) {
...
function getValidatedAmount(amount) {
  amount = amount.trim();
  if (amount.toLowerCase().endsWith('tigc')) {
...
function txLink(txId) {
  return 'http://explorer.foundchain.io/tx/' + txId;
}
