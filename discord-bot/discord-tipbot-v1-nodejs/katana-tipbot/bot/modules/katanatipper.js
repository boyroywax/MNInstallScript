...
let walletConfig = config.get('katanad');
...
function doBalance(message, tipper) {
  boxy.getBalance(tipper, 1, function(err, balance) {
    if (err) {
      message.reply('Error getting Katana Coin balance.').then(message => message.delete(5000));
    } else {
      message.reply('You have **' + balance + ' Katana (K2X) Coin** :moneybag:');
    }
  });
}
...
function doDeposit(message, tipper) {
  getAddress(tipper, function(err, address) {
    if (err) {
      message.reply('Error getting your Katana Coin deposit address.').then(message => message.delete(5000));
    } else {
      message.reply('Your Katana Coin (K2X) address is ' + address);
    }
  });
}
...
function doWithdraw(message, tipper, words, helpmsg, cmdOffset) {
  ...
  if (amount === null) {
    message.reply("I don't know how to withdraw that many Katana (K2X) Coins...").then(message => message.delete(5000));
    return;
  }
    ...
    if (amount < balance) {
      boxy.sendFrom(tipper, address, Number(amount-0.00001), function(err, txId) {
        if (err) {
          ...
          message.reply('You withdrew ' + amount + ' K2X to ' + address + '\n' + txLink(txId) + '\n');
        }
      });
    } else {
...
function doTip(bot, message, tipper, words, helpmsg, cmdOffset) {
  ...
  if (amount === null) {
    message.reply("I don't know how to tip that many Katana (K2X) Coins...").then(message => message.delete(5000));
    return;
  }
...
function doSoakRainDrizzle(bot, message, tipper, words, helpmsg, tipType, cmdOffset) {
  ...
  if (amount === null) {
    ...
    message.reply("Atleast 0.01 K2X is required to rain").then(message => message.delete(5000));
    return;
  }
...
function soak(amount, online, callback){
  ...
  setUsernames.push(onlineUsername);
  callback(onlineID, onlineID.length + " users currently online. No Katana Coin is rained",
      ":thunder_cloud_rain: Katana Coins are falling from the sky!!! :thunder_cloud_rain: \n**" +
      amount/onlineID.length + " K2X** soaked on " + setUsernames[0], setUsernames);
}
...
function rain(amount, online, message, callback){
  ...
  callback(onlineID, "No new messages, since I woke up", ":cloud_rain: Katana Coins are raining from the sky!!! :cloud_rain: \n" +
      "**" + amount/onlineID.length + " K2X** rained down on " + onlineUsername + " :rocket:");
}
...
function drizzle(amount, online, message, callback){
  ...
  callback(onlineID,"No new messages, since I woke up", ":white_sun_rain_cloud: Katana Coins are drizzling from the sky!!! :white_sun_rain_cloud: \n" +
      "**" + amount/onlineID.length + " K2X** rained down on " + onlineUsername + " :rocket:");
}


function getPrice(bot, msg){
  getDataFromAPI("https://cryptohub.online/api/market/ticker/K2X/", true, function(data){
    let result = JSON.parse(data);
    if (result !== "undefined") {
      if (result.BTC_K2X) {
        sendEmbedMessages(msg, "", 3447003, [{
          name: "Cryptohub - K2X Price",
          value: "**" + result.BTC_K2X.last + "** BTC/K2X"
        }])
      } else {
...
function getBlock(bot, msg){
  getDataFromAPI("http://159.65.75.222:3001/api/getblockcount", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0x00AE86, [{
          name: "Katana (K2X) Coin - Current BLOCK",
...
function getWeight(bot, msg){
  getDataFromAPI("http://159.65.75.222:3001/api/getdifficulty", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0xF55555, [{
          name: "Katana (K2X) Coin - Current Staking weight",
...
function getSupply(bot, msg){
  getDataFromAPI("http://159.65.75.222:3001/ext/getmoneysupply", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0xF55555, [{
          name: "Katana (K2X) Coin - Supply",
          value: "Supply: **" + result + "** Katana (K2X) Coins"
        }])
      } else {
        msg.reply("Price is not set by buyers/sellers")
      }
...
function sendTIGC(bot, message, tipper, recipient, amount, privacyFlag) {
        ...
        if (err) {
          ...
          if (privacyFlag) {
            ...
            var iimessage =
              ...
              txLink(txId) +
              '\n' +
              'DM me `$help` for KatanaCoinTipper instructions.';
...
function txLink(txId) {
  return 'http://159.65.75.222:3001/tx/' + txId;
}
