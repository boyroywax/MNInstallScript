...
function doBalance(message, tipper) {
  boxy.getBalance(tipper, 1, function(err, balance) {
    if (err) {
      message.reply('Error getting Legion Coin balance.').then(message => message.delete(5000));
    } else {
      message.reply('You have **' + balance + ' Legion Coin** :moneybag:');
    }
  });
}
...
function doDeposit(message, tipper) {
  getAddress(tipper, function(err, address) {
    if (err) {
      message.reply('Error getting your Legion Coin deposit address.').then(message => message.delete(5000));
    } else {
      message.reply('Your Legion (LEGN) address is ' + address);
    }
  });
}
...
function doWithdraw(message, tipper, words, helpmsg, cmdOffset) {
  ...
  if (amount === null) {
    message.reply("I don't know how to withdraw that many Legion Coins...").then(message => message.delete(5000));
    return;
  }
    ...
    if (amount < balance) {
      boxy.sendFrom(tipper, address, Number(amount-0.00001), function(err, txId) {
        if (err) {
          ...
          message.reply('You withdrew ' + amount + ' Legion (LEGN) to ' + address + '\n' + txLink(txId) + '\n');
        }
      });
    } else {
...
function doTip(bot, message, tipper, words, helpmsg, cmdOffset) {
  ...
  if (amount === null) {
    message.reply("I don't know how to tip that many  Legion Coins...").then(message => message.delete(5000));
    return;
  }
  ...
  if (message.mentions.users.first().id) {
    boxy.getBalance(tipper, function(err, balance) {
      if (amount < balance) {
        sendLEGN(bot, message, tipper, message.mentions.users.first().id.replace('!', ''), amount, prv);
      } else {
        message.reply("Account has insufficient funds").then(message => message.delete(5000));
      }
    });
...
function doSoakRainDrizzle(bot, message, tipper, words, helpmsg, tipType, cmdOffset) {
  ...
  if (amount === null) {
    ...
    message.reply("Atleast 0.01 LEGN is required to rain").then(message => message.delete(5000));
    return;
  }
...
function soak(amount, online, callback){
  ...
  setUsernames.push(onlineUsername);
  callback(onlineID, onlineID.length + " users currently online. No Legion Coin is rained",
      ":thunder_cloud_rain: Legion Coins are falling from the sky!!! :thunder_cloud_rain: \n**" +
      amount/onlineID.length + " LEGN** soaked on " + setUsernames[0], setUsernames);
}
...
function rain(amount, online, message, callback){
  ...
  callback(onlineID, "No new messages, since I woke up", ":cloud_rain: Legion Coins are raining from the sky!!! :cloud_rain: \n" +
      "**" + amount/onlineID.length + " LEGN** rained down on " + onlineUsername + " :rocket:");
}
...
function drizzle(amount, online, message, callback){
  ...
  callback(onlineID,"No new messages, since I woke up", ":white_sun_rain_cloud: Legion Coins are drizzling from the sky!!! :white_sun_rain_cloud: \n" +
      "**" + amount/onlineID.length + " LEGN** rained down on " + onlineUsername + " :rocket:");
}


function getPrice(bot, msg){
  getDataFromAPI("https://api.coingecko.com/api/v3/coins/legion-coin", true, function(data){
    let result = JSON.parse(data);
    if (result !== "undefined") {
      if (result.market_data) {
        sendEmbedMessages(msg, "", 3447003, [{
          name: "Legion Market Price",
          value: "**" + result.market_data.current_price.btc + "** BTC/LEGN"
        }])
      } else {
...
function getBlock(bot, msg){
  getDataFromAPI("http://173.249.1.107:3001/api/getblockcount", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0x00AE86, [{
          name: "Legion Coin - Current BLOCK",
...
function getWeight(bot, msg){
  getDataFromAPI("http://173.249.1.107:3001/api/getdifficulty", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0xF55555, [{
          name: "Legion Coin - Current Staking weight",
...
function getSupply(bot, msg){
  getDataFromAPI("http://173.249.1.107:3001/ext/getmoneysupply", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0xF55555, [{
          name: "Legion Coin - Supply",
          value: "Supply: **" + result + "** PRiVCYCoins"
        }])
      } else {
        msg.reply("Price is not set by buyers/sellers")
      }
...
function sendLEGN(bot, message, tipper, recipient, amount, privacyFlag) {
        ...
        if (err) {
          ...
          if (privacyFlag) {
            ...
            var iimessage =
              ...
              txLink(txId) +
              '\n' +
              'DM me `$help` for LegionCoinTipper instructions.';
...
function txLink(txId) {
  return 'http://legion.thecryptochat.net/tx.php?tx=' + txId;
}
