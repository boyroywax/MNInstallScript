...
function doBalance(message, tipper) {
  boxy.getBalance(tipper, 1, function(err, balance) {
    if (err) {
      message.reply('Error getting PRiVCY Coin balance.').then(message => message.delete(5000));
    } else {
      message.reply('You have **' + balance + ' PRiVCY Coin** :moneybag:');
    }
  });
}
...
function doDeposit(message, tipper) {
  getAddress(tipper, function(err, address) {
    if (err) {
      message.reply('Error getting your PRiVCY Coin deposit address.').then(message => message.delete(5000));
    } else {
      message.reply('Your PRiVCY Coin (PRIV) address is ' + address);
    }
  });
}
...
function doWithdraw(message, tipper, words, helpmsg, cmdOffset) {
  ...
  if (amount === null) {
    message.reply("I don't know how to withdraw that many PRiVCY Coins...").then(message => message.delete(5000));
    return;
  }
    ...
    if (amount < balance) {
      boxy.sendFrom(tipper, address, Number(amount-0.00001), function(err, txId) {
        if (err) {
          ...
          message.reply('You withdrew ' + amount + ' PRIV to ' + address + '\n' + txLink(txId) + '\n');
        }
      });
    } else {
...
function doTip(bot, message, tipper, words, helpmsg, cmdOffset) {
  ...
  if (amount === null) {
    message.reply("I don't know how to tip that many  PRiVCY Coins...").then(message => message.delete(5000));
    return;
  }
  ...
  if (message.mentions.users.first().id) {
    boxy.getBalance(tipper, function(err, balance) {
      if (amount < balance) {
        sendPRIV(bot, message, tipper, message.mentions.users.first().id.replace('!', ''), amount, prv);
      } else {
        message.reply("Account has insufficient funds").then(message => message.delete(5000));
      }
    });
...
function doSoakRainDrizzle(bot, message, tipper, words, helpmsg, tipType, cmdOffset) {
  ...
  if (amount === null) {
    ...
    message.reply("Atleast 0.50 PRIV is required to rain").then(message => message.delete(5000));
    return;
  }
...
function soak(amount, online, callback){
  ...
  setUsernames.push(onlineUsername);
  callback(onlineID, onlineID.length + " users currently online. No PRiVCY Coin is rained",
      ":thunder_cloud_rain: PRiVCY Coins are falling from the sky!!! :thunder_cloud_rain: \n**" +
      amount/onlineID.length + " PRIV** soaked on " + setUsernames[0], setUsernames);
}
...
function rain(amount, online, message, callback){
  ...
  callback(onlineID, "No new messages, since I woke up", ":cloud_rain: PRiVCY Coins are raining from the sky!!! :cloud_rain: \n" +
      "**" + amount/onlineID.length + " PRIV** rained down on " + onlineUsername + " :rocket:");
}
...
function drizzle(amount, online, message, callback){
  ...
  callback(onlineID,"No new messages, since I woke up", ":white_sun_rain_cloud: PRiVCY Coins are drizzling from the sky!!! :white_sun_rain_cloud: \n" +
      "**" + amount/onlineID.length + " PRIV** rained down on " + onlineUsername + " :rocket:");
}

function getPrice(bot, msg){
  getDataFromAPI("https://api.coingecko.com/api/v3/coins/privcy", true, function(data){
    let result = JSON.parse(data);
    if (result !== "undefined") {
      if (result.market_data) {
        sendEmbedMessages(msg, "", 3447003, [{
          name: "PRiVCY Market Price",
          value: "**" + result.market_data.current_price.btc + "** BTC/PRIV"
        }])
      } else {
...
function getBlock(bot, msg){
  getDataFromAPI("https://explorer.privcy.io/api/getblockcount", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0x00AE86, [{
          name: "PRiVCY Coin - Current BLOCK",
...
function getWeight(bot, msg){
  getDataFromAPI("https://explorer.privcy.io/api/getdifficulty", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0xF55555, [{
          name: "PRiVCY Coin - Current Staking weight",
...
function getSupply(bot, msg){
  getDataFromAPI("https://explorer.privcy.io/ext/getmoneysupply", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0xF55555, [{
          name: "PRiVCY Coin - Supply",
...
function sendPRIV(bot, message, tipper, recipient, amount, privacyFlag) {
        ...
        if (err) {
          ...
          if (privacyFlag) {
            ...
            var iimessage =
              ...
              txLink(txId) +
              '\n' +
              'DM me `$help` for PRiVCYCoinTipper instructions.';
...
function txLink(txId) {
  return 'https://explorer.privcy.io/tx/' + txId;
}
