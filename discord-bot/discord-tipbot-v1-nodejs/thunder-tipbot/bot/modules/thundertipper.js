...
let walletConfig = config.get('thunderstaked');
...
function doBalance(message, tipper) {
  boxy.getBalance(tipper, 1, function(err, balance) {
    if (err) {
      message.reply('Error getting Thunderstake (TSC) Coin balance.').then(message => message.delete(5000));
    } else {
      message.reply('You have **' + balance + ' Thunderstake (TSC) Coin** :moneybag:');
    }
  });
}
...
function doDeposit(message, tipper) {
  getAddress(tipper, function(err, address) {
    if (err) {
      message.reply('Error getting your Thunderstake (TSC) Coin deposit address.').then(message => message.delete(5000));
    } else {
      message.reply('Your Thunderstake (TSC) address is ' + address);
    }
  });
}
...
function doWithdraw(message, tipper, words, helpmsg, cmdOffset) {
  ...
  if (amount === null) {
    message.reply("I don't know how to withdraw that many Thunderstake (TSC) Coins...").then(message => message.delete(5000));
    return;
  }
    ...
    if (amount < balance) {
      boxy.sendFrom(tipper, address, Number(amount-0.00001), function(err, txId) {
        if (err) {
          ...
          message.reply('You withdrew ' + amount + ' Thunderstake (TSC) to ' + address + '\n' + txLink(txId) + '\n');
        }
      });
    } else {
...
function doTip(bot, message, tipper, words, helpmsg, cmdOffset) {
  ...
  if (amount === null) {
    message.reply("I don't know how to tip that many Thunderstake (TSC) Coins...").then(message => message.delete(5000));
    return;
  }
...
function doSoakRainDrizzle(bot, message, tipper, words, helpmsg, tipType, cmdOffset) {
  ...
  if (amount === null) {
    ...
    message.reply("Atleast 0.1 Thunderstake (TSC) is required to rain").then(message => message.delete(5000));
    return;
  }
...
function soak(amount, online, callback){
  ...
  setUsernames.push(onlineUsername);
  callback(onlineID, onlineID.length + " users currently online. No Thunderstake (TSC) Coin is rained",
      ":thunder_cloud_rain: Thunderstake (TSC) Coins are falling from the sky!!! :thunder_cloud_rain: \n**" +
      amount/onlineID.length + " Thunderstake (TSC)** soaked on " + setUsernames[0], setUsernames);
}
...
function rain(amount, online, message, callback){
  ...
  callback(onlineID, "No new messages, since I woke up", ":cloud_rain: Thunderstake (TSC) Coins are raining from the sky!!! :cloud_rain: \n" +
      "**" + amount/onlineID.length + " Thunderstake (TSC)** rained down on " + onlineUsername + " :rocket:");
}
...
function drizzle(amount, online, message, callback){
  ...
  callback(onlineID,"No new messages, since I woke up", ":white_sun_rain_cloud: Thunderstake (TSC) Coins are drizzling from the sky!!! :white_sun_rain_cloud: \n" +
      "**" + amount/onlineID.length + " Thunderstake (TSC)** rained down on " + onlineUsername + " :rocket:");
}


function getPrice(bot, msg){
  getDataFromAPI("https://api.coingecko.com/api/v3/coins/thunderstake", true, function(data){
    let result = JSON.parse(data);
    if (result !== "undefined") {
      if (result.market_data) {
        sendEmbedMessages(msg, "", 3447003, [{
          name: "Thunderstake (TSC) Market Price",
          value: "**" + result.market_data.current_price.btc + "** BTC/TSC"
        }])
      } else {
...
function getBlock(bot, msg){
  getDataFromAPI("http://tsc-explorer.giize.com/api/getblockcount", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0x00AE86, [{
          name: "Thunderstake (TSC) Coin - Current BLOCK",
...
function getWeight(bot, msg){
  getDataFromAPI("http://tsc-explorer.giize.com/api/getdifficulty", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0xF55555, [{
          name: "Thunderstake (TSC) Coin - Current Staking weight",
...
function getSupply(bot, msg){
  getDataFromAPI("http://tsc-explorer.giize.com/ext/getmoneysupply", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0xF55555, [{
          name: "Thunderstake (TSC) Coin - Supply",
          value: "Supply: **" + result + "** TSC Coins"
        }])
      } else {
        msg.reply("Price is not set by buyers/sellers")
      }
        ...
        if (err) {
          ...
          if (privacyFlag) {
            ...
            var iimessage =
              ...
              txLink(txId) +
              '\n' +
              'DM me `$help` for Thunderstake (TSC) Coin Tipper instructions.';
...
function txLink(txId) {
  return 'http://tsc-explorer.giize.com/tx/' + txId;
}
