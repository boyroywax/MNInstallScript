...
let walletConfig = config.get('touchd');
...
function doBalance(message, tipper) {
  boxy.getBalance(tipper, 1, function(err, balance) {
    if (err) {
      message.reply('Error getting Touch Coin balance.').then(message => message.delete(5000));
    } else {
      message.reply('You have **' + balance + ' Touch Coin** :moneybag:');
    }
  });
}
...
function doDeposit(message, tipper) {
  getAddress(tipper, function(err, address) {
    if (err) {
      message.reply('Error getting your Touch Coin deposit address.').then(message => message.delete(5000));
    } else {
      message.reply('Your Touch (TOUCH) address is ' + address);
    }
  });
}
...
function doWithdraw(message, tipper, words, helpmsg, cmdOffset) {
  ...
  if (amount === null) {
    message.reply("I don't know how to withdraw that many Touch Coins...").then(message => message.delete(5000));
    return;
  }
    ...
    if (amount < balance) {
      boxy.sendFrom(tipper, address, Number(amount-0.00001), function(err, txId) {
        if (err) {
          ...
          message.reply('You withdrew ' + amount + ' Touch (TOUCH) to ' + address + '\n' + txLink(txId) + '\n');
        }
      });
    } else {
...
function doTip(bot, message, tipper, words, helpmsg, cmdOffset) {
  ...
  if (amount === null) {
    message.reply("I don't know how to tip that many Touch Coins...").then(message => message.delete(5000));
    return;
  }
...
function doSoakRainDrizzle(bot, message, tipper, words, helpmsg, tipType, cmdOffset) {
  ...
  if (amount === null) {
    ...
    message.reply("Atleast 0.1 TOUCH is required to rain").then(message => message.delete(5000));
    return;
  }
...
function soak(amount, online, callback){
  ...
  setUsernames.push(onlineUsername);
  callback(onlineID, onlineID.length + " users currently online. No Touch Coin is rained",
      ":thunder_cloud_rain: Touch Coins are falling from the sky!!! :thunder_cloud_rain: \n**" +
      amount/onlineID.length + " TOUCH** soaked on " + setUsernames[0], setUsernames);
}
...
function rain(amount, online, message, callback){
  ...
  callback(onlineID, "No new messages, since I woke up", ":cloud_rain: Touch Coins are raining from the sky!!! :cloud_rain: \n" +
      "**" + amount/onlineID.length + " TOUCH** rained down on " + onlineUsername + " :rocket:");
}
...
function drizzle(amount, online, message, callback){
  ...
  callback(onlineID,"No new messages, since I woke up", ":white_sun_rain_cloud: Touch Coins are drizzling from the sky!!! :white_sun_rain_cloud: \n" +
      "**" + amount/onlineID.length + " TOUCH** rained down on " + onlineUsername + " :rocket:");
}


function getPrice(bot, msg){
  getDataFromAPI("https://start-ex.com/api/market/31", true, function(data){
    let result = JSON.parse(data);
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 3447003, [{
          name: "Touch Market Price",
          value: "**" + result.market.last_sell_price + "** BTC/TOUCH"
        }])
      } else {
...
function getBlock(bot, msg){
  getDataFromAPI("http://explorer.touch.money/api/getblockcount", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0x00AE86, [{
          name: "Touch Coin - Current BLOCK",
...
function getWeight(bot, msg){
  getDataFromAPI("http://explorer.touch.money/api/getdifficulty", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0xF55555, [{
          name: "Touch Coin - Current Staking weight",
...
function getSupply(bot, msg){
  getDataFromAPI("http://explorer.touch.money/ext/getmoneysupply", true, function(result){
    if (result !== "undefined") {
      if (result) {
        sendEmbedMessages(msg, "", 0xF55555, [{
          name: "Touch Coin - Supply",
          value: "Supply: **" + result + "** TOUCH Coins"
        }])
      } else {
        msg.reply("Price is not set by buyers/sellers")
      }
        ...
        if (err) {
          ...
          if (privacyFlag) {
            ...
            var iimessage =
              ...
              txLink(txId) +
              '\n' +
              'DM me `$help` for TouchCoinTipper instructions.';
...
function txLink(txId) {
  return 'http://explorer.touch.money/tx/' + txId;
}
