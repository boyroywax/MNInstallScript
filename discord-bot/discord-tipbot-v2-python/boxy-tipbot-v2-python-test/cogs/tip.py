...
class Tip:
    ...
    async def tip(self, ctx, user:discord.Member, amount:float):
        ...
        if float(balance) < amount:
            await self.bot.say("{} **:warning:You cannot tip more BOXY than you have!:warning:**".format(ctx.message.author.mention))
        else:
            mysql.add_tip(snowflake, tip_user, amount)
            await self.bot.say("{} **Tipped {} {} BOXY! ** REMINDER: Do not store your coins on this server - they are not backed up and can be lost at any point! The administrator of this channel is not responsible for lost coins. You have been warned!".format(ctx.message.author.mention, user.mention, str(amount)))
