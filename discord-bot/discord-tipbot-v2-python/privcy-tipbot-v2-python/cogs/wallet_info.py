...
class Wallet:
    ...
    async def wallet(self):
        ...
        # staking_status = info["staking status"]
        ...
        embed.add_field(name="Balance", value="{:.8f} {}".format(wallet_balance, self.currency_symbol))
        embed.add_field(name="Connections", value=connection_count)
        embed.add_field(name="Block Height", value=block_height)
        embed.add_field(name="Coin Supply", value="{:.8f}".format(money_supply))
        embed.add_field(name="Transaction Fee", value="{:.8f}".format(paytxfee))
