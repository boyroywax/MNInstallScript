...
def run():
    with warnings.catch_warnings():
        ...
        cursor.execute("""CREATE TABLE IF NOT EXISTS masternode (
            snowflake_fk BIGINT UNSIGNED NOT NULL,
            snowflake_mn_fk BIGINT UNSIGNED NOT NULL,
            amount DECIMAL(20, 8) NOT NULL,
            FOREIGN KEY (snowflake_fk) REFERENCES users(snowflake_pk),
            FOREIGN KEY (snowflake_mn_fk) REFERENCES users(snowflake_pk)
            )""")
        cursor.execute("""CREATE TABLE IF NOT EXISTS swap (
            swap_id BIGINT UNSIGNED NOT NULL,
            snowflake_from_fk BIGINT UNSIGNED NOT NULL,
            snowflake_to_fk BIGINT UNSIGNED NOT NULL,
            amount DECIMAL(20, 8) NOT NULL,
            currency VARCHAR(20) NOT NULL,
            currency_amount DECIMAL(20, 8) NOT NULL,
            currency_address VARCHAR(34),
            status VARCHAR(20) NOT NULL,
            swap_time DATETIME,
            FOREIGN KEY (snowflake_from_fk) REFERENCES users(snowflake_pk),
            FOREIGN KEY (snowflake_to_fk) REFERENCES users(snowflake_pk),
